...
static int imq_dev_xmit2(struct sk_buff *skb, struct net_device *dev)
{
	...
	struct net_device_stats *stats = (struct net_device_stats*) dev->ml_priv;
	
	if ( skb->mark & QOS_DEFAULT_MARK ) {
		skb->mark &= ~QOS_DEFAULT_MARK;
		//skb->imq_flags |= QOS_DEV_IMQ1;
		//skb->imq_flags |= IMQ_F_ENQUEUE;
		skb->dev = __dev_get_by_index(&init_net, skb->skb_iif );
		if ( !skb->dev ) {
			dev_kfree_skb( skb );
			stats->tx_dropped++;			
			return 0;
...
static int __init imq_dev_init(struct net_device *dev)
{
	...
	dev->ml_priv = kmalloc(sizeof(struct imq_private), GFP_KERNEL);
	if (dev->ml_priv == NULL)
		return -ENOMEM;
	memset(dev->ml_priv, 0, sizeof(struct imq_private));
...
static const struct net_device_ops imq_netdev_ops = {
	...
	.ndo_init			= imq_dev_init,
	.ndo_uninit			= imq_dev_uninit,
};
