...
static int br_set_rm_vlantag(struct net_bridge *br, unsigned long val)
{
	br->rm_vlantag = val ? true : false;
	return 0;
}

static ssize_t show_rm_vlantag(struct device *d,
				     struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%d\n", br->rm_vlantag);
...
static ssize_t store_rm_vlantag(struct device *d,
				      struct device_attribute *attr,
				      const char *buf, size_t len)
{
	return store_bridge_parm(d, buf, len, br_set_rm_vlantag);
}
static DEVICE_ATTR(rm_vlantag, S_IRUGO | S_IWUSR, show_rm_vlantag,
		   store_rm_vlantag);
#endif
